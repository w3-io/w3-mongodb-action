  ...
  return async () => {
    ...
    const result = await withClient(inputs.url, async (client) => {
      ...
      if (!inputs.collectionName) {
        throw new W3ActionError('MISSING_INPUT', 'Collection name required for this command')
      }
...
const router = createCommandRouter({
  ...
  'create-collection': dbHandler(async (db, _client, inputs) => {
    if (!inputs.collectionName)
      throw new W3ActionError('MISSING_INPUT', 'collection is required for create-collection')
  ...
  'drop-collection': dbHandler(async (db, _client, inputs) => {
    if (!inputs.collectionName)
      throw new W3ActionError('MISSING_INPUT', 'collection is required for drop-collection')
  ...
  'rename-collection': dbHandler(async (db, _client, inputs) => {
    if (!inputs.collectionName)
      throw new W3ActionError('MISSING_INPUT', 'collection is required for rename-collection')
    if (!inputs.newCollectionName)
      throw new W3ActionError('MISSING_INPUT', 'new-name is required for rename-collection')
  ...
  'run-command': dbHandler(async (db, _client, inputs) => {
    if (!inputs.commandStr) throw new W3ActionError('MISSING_INPUT', 'db-command is required for run-command')
  ...
  transaction: dbHandler(async (db, client, inputs) => {
    if (!inputs.operationsStr)
      throw new W3ActionError('MISSING_INPUT', 'operations is required for transaction')
    ...
    try {
      await session.withTransaction(async () => {
        for (const op of txOps) {
          if (!op.collection)
            throw new W3ActionError('MISSING_INPUT', 'Each transaction operation must specify a collection')
          if (!op.op) throw new W3ActionError('MISSING_INPUT', 'Each transaction operation must specify an op')
          const coll = db.collection(op.collection)

          switch (op.op) {
            case 'insertOne': {
              if (!op.document) throw new W3ActionError('MISSING_INPUT', 'insertOne requires document')
            ...
            case 'insertMany': {
              if (!op.documents || !Array.isArray(op.documents))
                throw new W3ActionError('MISSING_INPUT', 'insertMany requires documents array')
            ...
            case 'updateOne': {
              if (!op.filter || !op.update)
                throw new W3ActionError('MISSING_INPUT', 'updateOne requires filter and update')
            ...
            case 'updateMany': {
              if (!op.filter || !op.update)
                throw new W3ActionError('MISSING_INPUT', 'updateMany requires filter and update')
            ...
            case 'replaceOne': {
              if (!op.filter || !op.document)
                throw new W3ActionError('MISSING_INPUT', 'replaceOne requires filter and document')
            ...
            case 'deleteOne': {
              if (!op.filter) throw new W3ActionError('MISSING_INPUT', 'deleteOne requires filter')
            ...
            case 'deleteMany': {
              if (!op.filter) throw new W3ActionError('MISSING_INPUT', 'deleteMany requires filter')
            ...
            case 'findOneAndUpdate': {
              if (!op.filter || !op.update)
                throw new W3ActionError('MISSING_INPUT', 'findOneAndUpdate requires filter and update')
              const opts = {
                session,
                upsert: op.upsert === true,
  ...
  distinct: collHandler(async (collection, _db, inputs) => {
    if (!inputs.field) throw new W3ActionError('MISSING_INPUT', 'field is required for distinct')
  ...
  'insert-one': collHandler(async (collection, _db, inputs) => {
    if (!inputs.documentStr)
      throw new W3ActionError('MISSING_INPUT', 'document is required for insert-one')
  ...
  'insert-many': collHandler(async (collection, _db, inputs) => {
    if (!inputs.documentsStr)
      throw new W3ActionError('MISSING_INPUT', 'documents is required for insert-many')
    const docs = JSON.parse(inputs.documentsStr)
    if (!Array.isArray(docs))
      throw new W3ActionError('MISSING_INPUT', 'documents must be a JSON array')
  ...
  'update-one': collHandler(async (collection, _db, inputs) => {
    if (!inputs.updateStr) throw new W3ActionError('MISSING_INPUT', 'update is required for update-one')
  ...
  'update-many': collHandler(async (collection, _db, inputs) => {
    if (!inputs.updateStr) throw new W3ActionError('MISSING_INPUT', 'update is required for update-many')
    const updateDoc = JSON.parse(inputs.updateStr)
  ...
  'replace-one': collHandler(async (collection, _db, inputs) => {
    if (!inputs.documentStr)
      throw new W3ActionError('MISSING_INPUT', 'document is required for replace-one')
  ...
  'find-one-and-update': collHandler(async (collection, _db, inputs) => {
    if (!inputs.updateStr)
      throw new W3ActionError('MISSING_INPUT', 'update is required for find-one-and-update')
    const updateDoc = JSON.parse(inputs.updateStr)
    const options = {
      upsert: inputs.upsert,
  ...
  'find-one-and-replace': collHandler(async (collection, _db, inputs) => {
    if (!inputs.documentStr)
      throw new W3ActionError('MISSING_INPUT', 'document is required for find-one-and-replace')
    const replacement = JSON.parse(inputs.documentStr)
    const options = {
      upsert: inputs.upsert,
      returnDocument: inputs.returnDocument === 'before' ? 'before' : 'after',
  ...
  'bulk-write': collHandler(async (collection, _db, inputs) => {
    if (!inputs.operationsStr)
      throw new W3ActionError('MISSING_INPUT', 'operations is required for bulk-write')
  ...
  aggregate: collHandler(async (collection, _db, inputs) => {
    if (!inputs.pipelineStr)
      throw new W3ActionError('MISSING_INPUT', 'pipeline is required for aggregate')
  ...
  'create-index': collHandler(async (collection, _db, inputs) => {
    if (!inputs.indexStr) throw new W3ActionError('MISSING_INPUT', 'index is required for create-index')
  ...
  'create-indexes': collHandler(async (collection, _db, inputs) => {
    if (!inputs.operationsStr)
      throw new W3ActionError('MISSING_INPUT', 'operations is required for create-indexes')
    const indexSpecs = JSON.parse(inputs.operationsStr)
    if (!Array.isArray(indexSpecs))
      throw new W3ActionError('MISSING_INPUT', 'operations must be a JSON array of index specs')
    return collection.createIndexes(indexSpecs)
  }),

  'drop-index': collHandler(async (collection, _db, inputs) => {
    if (!inputs.indexName)
      throw new W3ActionError('MISSING_INPUT', 'index-name is required for drop-index')
